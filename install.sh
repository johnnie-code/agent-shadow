...
if [ "$USE_UV" = true ]; then
    ...
    if [ "$IS_TERMUX" = true ]; then
        uv pip install pydantic-core --extra-index-url https://eutalix.github.io/android-pydantic-core/
        uv pip install -e . --extra-index-url https://eutalix.github.io/android-pydantic-core/
    else
        uv pip install -e .
    fi
else
    ...
    if [ "$IS_TERMUX" = true ]; then
        ...
        .venv/bin/pip install -e . --extra-index-url https://eutalix.github.io/android-pydantic-core/
    else
...
if $VENV_PYTHON -c "import pydantic, pydantic_settings, fastapi, uvicorn, rich, watchdog, httpx, typer" &>/dev/null; then
    ...
    $VENV_PYTHON -c "import pydantic, pydantic_settings, fastapi, uvicorn, rich, watchdog, httpx, typer" || true
    exit 1
fi
