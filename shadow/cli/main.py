        ...
        if choice == "1":
            ...
            while True:
                ...
                if new_provider in ["mock", "openai", "anthropic", "gemini", "ollama"]:
                    break
                console.print("[red]Invalid AI Provider. Please enter one of: mock, openai, anthropic, gemini, ollama.[/red]")
            config_set_env("default_provider", new_provider)
            if new_provider != "mock":
                new_key = typer.prompt(f"Enter {new_provider.upper()} API Key (leave empty to skip)", default="", hide_input=True)
                if new_key:
            ...
            while True:
                ...
                if new_pref in ["terminal", "android", "none"]:
                    break
                console.print("[red]Invalid Notification Mode. Please enter one of: terminal, android, none.[/red]")
            ...
            while True:
                new_limit = typer.prompt("Enter Battery Saver Limit %", default=str(config.battery_limit)).strip()
                if new_limit.isdigit() and 0 <= int(new_limit) <= 100:
                    break
                console.print("[red]Invalid Battery Saver Limit. Please enter a positive integer between 0 and 100.[/red]")
